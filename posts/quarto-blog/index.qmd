...
Seven years ago, I first published a [Jekyll](https://jekyllrb.com/) blog hosted in Github pages and let it lie largley dormant for two years. 
...
This required the use of [git submodules](https://git-scm.com/book/en/v2/Git-Tools-Submodules) and memorization of Pelican commands to generate the content.
I forgot them often enough that I ended up using the README of the source repo to add notes for myself so I wouldn't forget how to publish.
...
depended on `nbconvert` to convert the notebooks into the html needed for publishing but after a recent version upgrade broke and soon even pinning the
older version no longer solved all the conflicts as other packages quickly required newer versions while the package was unable to keep up.
...
locally in VS Code, eliminate the need for a second repo, and the themes and layouts were based on simple [bootstrap](https://getbootstrap.com/docs/5.0/getting-started/introduction/).
...
Now Github was my personal blog CMS without any need for a database or admin site. Just simple `git commit`. 

Now I have one less excuse to start writing more and much more functionality to play with. I'm very excited about exploring more interactive data viz using [Observable](https://quarto.org/docs/computations/ojs.html).
